...
public class MonstersTest {
	...
	@Test
	public void getSerialID_Identity() {
		Monsters tmp = new Monsters(100, 10, ElementTypes.Air_Type, false,
				AppearanceTypes.Standard_Type, 100);
		assertTrue(tmp.getSerialID() == tmp.getSerialID());
	}

	@Test
	public void getSerialID_Persistance() {
		Random ran = new Random();
		Monsters tmp = new Monsters(100, 10, ElementTypes.Air_Type, false,
				AppearanceTypes.Standard_Type, 100);
		long before = tmp.getSerialID();
		long after = 0;
		int end = 0;
	...
	@Test
	public void getSerialID_DifferentMonsters() {
		Monsters m1 = new Monsters(100, 10, ElementTypes.Air_Type, false,
				AppearanceTypes.Standard_Type, 100);
		Monsters m2 = new Monsters(100, 10, ElementTypes.Air_Type, false,
				AppearanceTypes.Standard_Type, 100);
		assertTrue(m1.getSerialID() != m2.getSerialID());
	}

	@Test
	public void getSerialID_IDsConstantlyChanging() {
		Random ran = new Random();
		Monsters tmp = new Monsters(100, 10, ElementTypes.Air_Type, false,
				AppearanceTypes.Standard_Type, 100);
		long before = 0;
		...
		for (int i = 0; i < end; i++) {
			before = tmp.getSerialID();
			tmp = new Monsters(100, 10, ElementTypes.Air_Type, false,
					AppearanceTypes.Standard_Type, 100);
			after = tmp.getSerialID();
			if (before >= after && after != 9998 && before != 9998) {
				fail();
			}
		}
		assertTrue(before < after);
